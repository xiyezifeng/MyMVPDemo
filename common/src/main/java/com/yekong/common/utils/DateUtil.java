...
public class DateUtil {
     ...
     */
    public static int getIntervalDays(String largeDate, String smallDate) {
        return getIntervalDays(StringToDate(largeDate), StringToDate(smallDate));
    }
     ...
     * 是否超越或等于当前时间
     */
    public static boolean getCurrentTime(String string){
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try
        {
            Date d1 = StringToDate(string);
            Date d2 = new Date();
    ...
    public static String getDateByMim(long time){
        Date date = new Date(time);
        return getDate(date);
    }

    public static String getTimeByMin(long time){
        Date date = new Date(time);
        return getTime(date);
    }

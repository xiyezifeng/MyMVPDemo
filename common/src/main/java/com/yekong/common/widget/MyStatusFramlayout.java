...
public class MyStatusFramlayout extends FrameLayout {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        if (eroorid != -1) {
            ...
            error_layout = View.inflate(context, R.layout.comm_view_error, null);
        }
        if (loadingid != -1) {
            loading_layout = View.inflate(context, loadingid, null);
        }else{
            loading_layout = View.inflate(context, R.layout.comm_view_loading, null);
        }
        if (emptyid != -1) {
            empty_layout = View.inflate(context, emptyid, null);
        }else{
            empty_layout = View.inflate(context, R.layout.comm_view_empty, null);
        }
        loadStatus = status;
    ...
    private void initPage(){
        ...
        error_layout.setOnClickListener(view -> {
            if (!listener.onErrorClick()) {
            }
        });
        empty_layout.setOnClickListener(view -> {
            if (!listener.onErrorClick()) {
            }
        });
    }
